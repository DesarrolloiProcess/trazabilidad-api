 ...
 *  ver `recipientName`/`recipientPhone` en cada `DeliverySeed`. `client-particular` agrupa a los pacientes
 *  sin convenio (pago particular), igual que hace el backend real cuando clienteNit llega vacío. */
...
interface DeliverySeed {
  ...
  address: string;
  /** Nombre y teléfono del paciente que recibe la entrega — no es el cliente/convenio (clientId). */
...
const seeds: DeliverySeed[] = [
  ...
  },
  {
    ...
    evidence: {
      ...
      receiverName: 'Ana Lucía Restrepo',
...
export const deliveries: DeliveryDto[] = seeds.map((seed) => ({
  ...
  alistamientoStartedAt: seed.status !== 'creado' ? (seed.createdAt ?? iso(-1, 6)) : null,
  alistamientoEndedAt: seed.status !== 'creado' ? (seed.createdAt ?? iso(-1, 6)) : null,
